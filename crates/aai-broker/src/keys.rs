...
impl SigningKeys {
    ...
    pub fn merge_previous_pems(&mut self, paths: &[String]) -> Result<(), BrokerError> {
        if paths.is_empty() {
            return Ok(());
        }
        let Some(keys) = self
            ...
            return Err(BrokerError::Config("jwks missing keys array".to_string()));
        };
        for path in paths {
            let pem = fs::read_to_string(path).map_err(|err| {
                BrokerError::Config(format!("reading previous signing key {path}: {err}"))
            })?;
            keys.push(jwk_from_pem(&pem).map_err(BrokerError::Config)?);
        }
        Ok(())
    }
}
